...
if ($mode == 'test') {
        ...
        foreach ($lines as $email) {
                ...
                try {
                        ...
                        if ($return_path && $return_path !== 'record') {
                        ...
                        // Replacing From and FromName with Fucntion Data
                        $ofrom2 = modifyData($ofrom2);
                        $ip_pair = modifyData($ip_pair);
                        ...
                        $mail->isHTML(true);
                        // Replacing Subject with Fucntion Data
                        $sub2 = modifyData($sub2);
                        $mail->Subject = $sub2;
                        ...
                        if (!$send) {
                                $err = $mail->ErrorInfo;
                                $ecn = $ecn + 1;
                                array_push($finalMessageIdArray,[$email, $ip, trim($message_id), "<font color = 'red'> Mailer Error : ".$err."</font>"]);
                                mysql_query("insert ignore into `svml`.`auto_script_test_status` (`svml_sendgrid_id`,`ip`,`email`,`msgid`,`mode`,`sent_status`) values ('$iid','$ip','$email','".trim($message_id)."','test','0')");
                        } else {
                                $scn = $scn + 1;
                                array_push($finalMessageIdArray, [$email, $ip, trim($message_id), "<font color = 'green'>Success</font>"]);
                                mysql_query("insert ignore into `svml`.`auto_script_test_status` (`svml_sendgrid_id`,`ip`,`email`,`msgid`,`mode`,`sent_status`) values ('$iid','$ip','$email','".trim($message_id)."','test','1')");
                        }
                } catch (phpmailerException $e) {
                        array_push($finalMessageIdArray,[$email, $ip, trim($message_id), "<font color = 'red'> Mailer Error : ".$e->errorMessage()."</font>"]);
                        mysql_query("insert ignore into `svml`.`auto_script_test_status` (`svml_sendgrid_id`,`ip`,`email`,`msgid`,`mode`,`sent_status`) values ('$iid','$ip','$email','".trim($message_id)."','test','0')");
                } catch (Exception $e) {
                        array_push($finalMessageIdArray,[$email, $ip, trim($message_id), "<font color = 'red'> Mailer Error : ".$e->getMessage()."</font>"]);
                        mysql_query("insert ignore into `svml`.`auto_script_test_status` (`svml_sendgrid_id`,`ip`,`email`,`msgid`,`mode`,`sent_status`) values ('$iid','$ip','$email','".trim($message_id)."','test','0')");
                }
        }
        echo serialize($finalMessageIdArray);
