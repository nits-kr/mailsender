...
while ($count < $total) {
	...
	foreach ($u as $linen) {
		...
		if ($ip_send_limit == $ip_mail_sen) {
			...
			@$de = date('Y-m-d');
			$cmd = "php /var/www/html/interface_new/maild_man_lu_new.php '$iid' '$toemails' '$linen' '$domain_map[$linen]'  >> /var/www/html/interface_new/out/$ip_pair.$de &";
			$res = exec($cmd);
	...
	if ($send == '1') {
		...
		$id2 = mysql_insert_id();
		$cmd = "php /var/www/html/interface_new/maild_man_lu_testmail22_new.php '$iid' '$toemails' '$smtp' '$id2' >> /var/www/html/interface_new/out/$ip_pair &";
		$res = exec($cmd);
		$send = 0;
		sleep(2);
