...
$limit = $c;
										    ...
										    @$de=date('Y-m-d');	
											$cmd="php /var/www/html/send_all/smtpgo_man.php $iid $toemails $linen >> /var/www/html/send_all/out/$ip_pair.$de &";
											$res=exec($cmd);
											sleep($wait_time);
											//echo "$res\n";
                         ...
                         /*          foreach($unique as $smtpip)
                                   ...
                                   $query=mysql_query("insert into auto_script_status (svml_id,status,log,reason) values ('$iid','0','halt','sending test email data')");
											 $id2=mysql_insert_id();
											$cmd="php /var/www/html/send_all/maild_man_lu_testmail_smtpgo.php $iid $toemails $smtp $id2 >> /var/www/html/send_all/out/$ip_pair &";
											$res=exec($cmd);
