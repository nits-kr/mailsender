...
if ($mode == 'test') {
        ...
        foreach ($lines as $email) {
                ...
                if($argv[4] == "record") {
                        $sent_status = is_null($err) ? 1 : 0;
                        $modifiedMessageId = str_replace(">","",str_replace("<","",$message_id));
                        $record = "insert ignore into `svml`.`auto_script_test_status` (`svml_sendgrid_id`,`ip`,`email`,`msgid`,`mode`,`sent_status`) values ('$iid','$smtpip','$email','$modifiedMessageId','bulk','$sent_status')";
                        mysql_query($record);
                }
        }
        echo $r = "Error : " . $err . "sent successfully to  " . $scn . "  Subscribers out of " . $cn . "  Subscribers from IP : $smtpip || offerid : " . $offer . " || Error : " . $ecn;
