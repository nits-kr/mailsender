...
//variable we can use script ,smode,remarks
$iid=mysql_insert_id();
foreach($u as $line)
                  	{
						$toemails=base64_encode($emailtest);
                   $cmd="php /var/www/html/send_all/maild_man_lu.php $iid $toemails $line";
						$result = exec($cmd);
						echo $result."<br>";
						}
						echo "done";  
...
mysql_query("update svml_sendgrid set in_link='$furl' where sno='$iid'");
                                   ...
                                   $tto=array();
										   ...
										   $linen=$u[$i];
											...
											$cmd="php /var/www/html/send_all/maild_man_lu.php '$iid' '$toemails' '$linen' >> /var/www/html/send_all/out/$ip_pair &";	
											exec($cmd);
											}
											sort($u);
											...
											{
											$toemails=base64_encode($emailtest);
											$cmd="php /var/www/html/send_all/maild_man_lu.php $iid $toemails $smtpip  >> /var/www/html/send_all/out/$ip_pair &";
											exec($cmd);
											 } 
											 	}
...
values  ('$ip_pair','$mode','$sub','$ofrom','$emailtest','$type','$msg','$limit','$ofrid','$data','$domain','$time_gap_to_send_val','$limit_to_send_val','$mailer','$textm','$times','$d',now(),'$ip','$msid','$users','$a[status]','$wait_time',
'$sencode','$fmencode','$charen','$contend','$mail_ch','$mail_per','$name','$inbp','$count')");
//variable we can use server  ,script ,smode,oid,remarks	
