...
function modifyData($input) {
    global $email, $nameemailid, $ip_pair, $ofrom2, $dateofmail, $message_id, $domain;
    $input = str_replace("{{email}}",$email,$input);
    $input = str_replace("{{name}}",$nameemailid,$input);
    $input = str_replace("{{fromid}}",$ip_pair,$input);
    $input = str_replace("{{fromname}}",$ofrom2,$input);
    $input = str_replace("{{date}}",$dateofmail,$input);
    $input = str_replace("{{msgid}}",$message_id,$input);
    $input = str_replace("{{domain}}",$domain,$input);
    $input = get_randomdata($input);
    return $input;
}
