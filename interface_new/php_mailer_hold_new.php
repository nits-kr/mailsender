...
include "/var/www/html/interface_new/send_approval_test.php";
...
$inbox_percentage =  trim($_REQUEST['inbox_percent']);
$retest_after =  trim($_REQUEST['retest_after']);
$datafile = "/var/www/data/$data";
$count = count($u);
$res_choice = trim($_REQUEST['res_choice']);
$relayp = "100"; //trim($_REQUEST['relayp']);
$sentResponseTable = null;
/*if($emailtest=='')
        {
...
if ($mode == "test") {
        ...
        if (($sen_t == 'manual') || ($sen_t == 'auto')) {
                ...
                $sentResponseTable .= '<thead><tr><th>Email</th><th>IP Address</th><th>Message ID</th><th>Sent Status</th><th>Sent Response</th></tr></thead>';
                $sentResponseTable .= '<tbody>';
                        foreach ($u as $line) {
                                $toemails = base64_encode($emailtest);
                                $response = `php /var/www/html/interface_new/advance_test.php '$iid' '$toemails' '$line'`;
                                $sentResponseTable .= arrayToHtmlTable($response);
                        }
                $sentResponseTable .= '</tbody>';
...
elseif ($mode == "bulk") // Bulk mailing 
{
        switch ($sen_t) {
                case "manual":
                        ...
                        $karan = mysql_query("select sno,status,reason from svml_sendgrid where `msg`like BINARY  '$msg' and `mode` = 'bulk' order by sno desc limit 1") or die(mysql_error());
                        ...
                        if ($a['status'] == '1') {
                                ...
                                for ($i = 0; $i < $ipc; $i++) {
                                        ...
                                        $cmd = "php /var/www/html/interface_new/maild_man_lu_new.php '$iid'  '$toemails' '$linen' >> /var/www/html/interface_new/out/" . $ip_pair . "_" . $d . " &";
                                        exec($cmd);
                                }
                                sort($u);
                ...
                break;
                        ...
                        #################################bulk auto ###############################################
                case "auto":
                        if ($inbox_percentage == NULL) {
                                $inbox_percentage = '100';
                        }
                        if ($retest_after == NULL) {
                                $retest_after = '500';
                        }
                        if ($retest_after > 5000) {
                                echo "Inbox Test After Maximum allowed value is 5000..! Please Reduce ..!";
                                exit;
                        }
                        $check_q = mysql_query("select sno,status,reason from svml_sendgrid where `subject`like BINARY  '$sub' and `ip`like BINARY  '$ip_pair' and `from_val`like BINARY  '$ofrom' and `msg`like BINARY  '$msg' and `domain` like BINARY  '$domain' and `bcc` like BINARY  '$msid' and `offer` like '$check[0]%' and `data` like '$data' and `limits` like BINARY '$limit' and `limit_to_send` like BINARY '$limit_to_send_val' and `sleep_time` like BINARY '$time_gap_to_send_val' and `sleep` like BINARY '$wait_time' and `mode` = 'bulk' order by sno desc limit 1 ") or die(mysql_error());
                        ...
                        if ($a["status"] == '1') {
                                ...
                                ################## check html match ###################
                                $msg_q = mysql_query("select * from svml_sendgrid  where `msg` like BINARY  '$msg' and `subject`like BINARY  '$sub' and `ip`like BINARY  '$ip_pair' and `from_val`like BINARY  '$ofrom' and status = 1 order by sno desc limit 1");
                                if ($msg_row = mysql_fetch_array($msg_q)) {
                                        mysql_query("insert into svml.svml_sendgrid (`ip`,`mode`,`subject`,`from_val`,`emails`,`type`,`msg`,`limits`,`offer`,`data`,`domain`,`sleep_time`,`limit_to_send`,`mailer`,`username`,`textm`,`times`,`date`,`timestamp`,`server`,`bcc`,`mutidomains`,`status`,`sleep`,`sencode`, `fencode` ,`charen` , `contend` ,`head`,`pwd`,`tempname`,`oid`,`stype`,`script`,`smode`,`relay_per`,`charen_alt`,`contend_alt`,`reply_to`,`xmailer`,`inbpatt`,`headers`,`reason`,`fbl`,`retest`,`retest_after`,`inbox_patt`)
                                                        values  ('$ip_pair','$mode','$sub','$ofrom','$emailtest','$type','$msg','$limit','$ofrid','$data','$domain','$time_gap_to_send_val','$limit_to_send_val','$mailer_id','$usermailer','$textm','$times','$d',now(),'$ip','$msid','$users','1','$wait_time',
                                                        '$sencode','$fmencode','$charen','$contend','$mail_ch','$mail_per','$name','$inbp','$count','$mailer_id','$res_choice','$relayp','$charen_alt','$contend_alt','$reply_to','$xmailer','$pattern','$headers','$reason','$fbl_choice','$retest','$retest_after','$inbox_percentage')");
                                        //variable we can use smode,remarks
                                        $iid = mysql_insert_id();
                                        $whatchanged = array();
                                        ...
                                        if ($data != $msg_row['data']) {
                                                array_push($whatchanged, 'domain');
                                                echo "DataFile  changed</br>";
                                        }
                                        ...
                                        ###########################Insert Into Table#####################
                                        mysql_query("insert into svml.svml_sendgrid (`ip`,`mode`,`subject`,`from_val`,`emails`,`type`,`msg`,`limits`,`offer`,`data`,`domain`,`sleep_time`,`limit_to_send`,`mailer`,`username`,`textm`,`times`,`date`,`timestamp`,`server`,`bcc`,`mutidomains`,`status`,`sleep`,
                                        `sencode`, `fencode` ,`charen` , `contend` ,`head`,`pwd`,`tempname`,`oid`,`stype`,`script`,`smode`,`relay_per`,`charen_alt`,`contend_alt`,`reply_to`,`xmailer`,`inbpatt`,`headers`,`reason`,`fbl`,`retest`,`retest_after`,`inbox_patt`)
                                        values  ('$ip_pair','$mode','$sub','$ofrom','$emailtest','$type','$msg','$limit','$ofrid','$data','$domain','$time_gap_to_send_val','$limit_to_send_val','$mailer_id','$usermailer','$textm','$times','$d',now(),'$ip','$msid','$users','1','$wait_time',
                                        '$sencode','$fmencode','$charen','$contend','$mail_ch','$mail_per','$name','$inbp','$count','$mailer_id','$sen_t','$relayp','$charen_alt','$contend_alt','$reply_to','$xmailer','$pattern','$headers','$reason','$fbl_choice','$retest','$retest_after','$inbox_percentage')");
                                        $iid = mysql_insert_id();
                                        $de = date('Y-m-d H:i:s');
                                        $status = 0;
                                        // Send Test Email to Approval Id's
                        ...
                        if (($mode == "bulk") && ($a["status"] == '1')) // Bulk mailing 
                        {
                                foreach ($u as $line) {
                                        ...
                                        mysql_query("insert into svml_ip_pool (svml_sendgrid_id,ip,status,data,ip_send_limit) values ('$a[sno]','$onlyip','Y','$data','$dis_file')");
                                        echo mysql_error();
                                }
                                echo "<br>To Run In Screen Use Below Command<br> <font color='red'>cd /var/www/html/interface_new/<br>";
                                echo "php php_mailer_auto_send_v2.php " . $a['sno'];
                                echo "</font>";
                        }
                        break;
                default:
