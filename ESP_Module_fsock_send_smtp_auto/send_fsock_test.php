...
@$mailing_ip = explode("\n",$campJsonData['mailing_ip']);
$lines = explode("|",$mailing_ip[0]);
$ip = $lines[0];
...
foreach($emailJsonData as $email)
{
    ...
    $sent_status = (strstr(trim($last_response), '250')) ? 1 : 0;
    $modifiedMessageId = str_replace(['<', '>'], '', $msgid);
    $record = "insert ignore into `svml`.`auto_script_test_status` (`svml_sendgrid_id`,`ip`,`email`,`msgid`,`mode`,`sent_status`) values ('$entity_id','$ip','$email','$modifiedMessageId','test','$sent_status')";
    mysql_query($record) or die(mysql_error());

    // Array push message id in finalMessageIdArray
    if ($sent_status === 1) {
        array_push($finalMessageIdArray, [$email, $ip, trim($msgid), "<font color = 'green'>Success</font>", $returnPath]);
    } else {
        array_push($finalMessageIdArray, [$email, $ip, trim($msgid), "<font color = 'red'>Failed</font>", $returnPath]);
    }
}
// if($ecn > 0)
