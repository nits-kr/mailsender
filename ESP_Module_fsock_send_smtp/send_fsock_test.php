...
// // Enable verbose logging for SSL/TLS
...
//             'SNI_enabled' => true,
//             'SNI_server_name' => 'vmi1756767.contaboserver.net', // Replace with your server's hostname
//             'crypto_method' => STREAM_CRYPTO_METHOD_TLSv1_2_CLIENT, // Adjust SSL/TLS version if needed
//             'ciphers' => 'HIGH:!SSLv2:!SSLv3:!TLSv1:!TLSv1.1' // Adjust cipher suites if needed
//         )
//     )
// );
include "include.php";
...
foreach($emailJsonData as $email)
{
    ...
    if (strpos($line, 'STARTTLS') !== false) {
        fputs($smtp, "STARTTLS\r\n");
        $line .= fgets($smtp, 1024); // Read STARTTLS response


        // Initiate TLS encryption
        stream_socket_enable_crypto($smtp, true, STREAM_CRYPTO_METHOD_TLSv1_2_CLIENT);

        // Re-greet the server after STARTTLS
        fputs($smtp, "EHLO $returnPath\r\n");
        $line .= fgets($smtp, 1024); // Read EHLO response
    }
...
$ip = $lines[0];
$log_query = "insert ignore into `report`.`sending_stats` (`mailer`,`template_id`,`interface`,`smtp`,`offer_id`,`domain`,`from`,`mode`,`sent`,`error`) values ('$mailer','$entity_id','FSOCK SEND SMTP','$ip','$offerId','$domain','$from_email','$mode','$scn','$ecn')";
