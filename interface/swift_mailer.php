...
foreach($u as $line)
                   ...
                   $cmd="php /var/www/html/interface/maild_man_lu_swift.php $iid $toemails $line";
						$result = exec($cmd);
						echo $result."<br>";
						}
						echo "done";  
...
mysql_query("update svml_sendgrid set in_link='$furl' where sno='$iid'");
                                   ...
                                   $tto=array();
										   ...
										   $linen=$u[$i];
											...
											$cmd="php /var/www/html/interface/maild_man_lu_swift.php '$iid' '$toemails' '$linen' >> /var/www/html/interface/out/$ip_pair &";	
											exec($cmd);
											}
											sort($u);
											...
											{
											$toemails=base64_encode($emailtest);
											$cmd="php /var/www/html/interface/maild_man_lu_swift.php $iid $toemails $smtpip  >> /var/www/html/interface/out/$ip_pair &";
											exec($cmd);
											 } 
											 	}
