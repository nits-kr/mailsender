...
//variable we can use server
$iid=mysql_insert_id();
foreach($u as $line)
                  	{
						$toemails=base64_encode($emailtest);
                   $cmd="php /var/www/html/interface/maild_man_lu.php $iid $toemails $line";
						$result = exec($cmd);
						echo $result."<br>";
						}
						echo "done";  
...
'$sencode','$fmencode','$charen','$contend','$mail_ch','$mail_per','$name','$inbp','$count','$mailer_id')");
//variable we can use server ,tempname ,script ,smode,stype,remarks
$iid=mysql_insert_id();					  
$furl=$url.$iid;
mysql_query("update svml_sendgrid set in_link='$furl' where sno='$iid'");
                                   ...
                                   $tto=array();
										   ...
										   $linen=$u[$i];
											...
											$cmd="php /var/www/html/interface/maild_man_lu.php '$iid' '$toemails' '$linen' >> /var/www/html/interface/out/$ip_pair &";	
											exec($cmd);
											}
											sort($u);
											...
											{
											$toemails=base64_encode($emailtest);
											$cmd="php /var/www/html/interface/maild_man_lu.php $iid $toemails $smtpip  >> /var/www/html/interface/out/$ip_pair &";
											exec($cmd);
											 } 
											 	}
...
echo "To Run In Screen Use Below Command<br>cd /var/www/html/interface/<br>";
