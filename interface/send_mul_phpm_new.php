...
$count1=trim($arr2['0']);
$retest = $arr['retest'];
$retest_after = $arr['retest_after'];
####################################new mail after
$mail_per_ip=round($mail_per/$intial_ip_count);
...
$limit = $c;
                                   ...
                                   while($rrow=mysql_fetch_array($ipss))
									...
									if($final_ip_count=='0') {	
										...
										$message = base64_encode("LIMIT REACHED IP COUNT :  ".$curent_ip['0'] ."  SPAM IP COUNT :  ".$curent_ip['1']."  HOLD IP COUNT :  ".$curent_ip['2']." | Script Stopped");
										`php /var/www/html/notification/send_notification.php $iid $message`;	
										// $ch = curl_init();
			...
			mysql_query("update svml_sendgrid set limits='$upd_svml_table' where sno='$iid'");
                                    ...
                                    foreach($u as $linen)											
											...
											$idss=rtrim($ids,"\n");
										   	...
										   	@$de=date('Y-m-d');	
											$cmd="php /var/www/html/interface/maild_man_lu_new.php '$iid' '$toemails' '$linen' '$domain_map[$linen]'  >> /var/www/html/interface/out/$ip_pair.$de &";
											$res=exec($cmd);
                                   ...
                                   if($testcount==$count)											 
									...
									$id2=mysql_insert_id();
									$cmd="php /var/www/html/interface/maild_man_lu_testmail22_new.php '$iid' '$toemails' '$smtp' '$id2' >> /var/www/html/interface/out/$ip_pair &";
									$res=exec($cmd);
									$send=0;	
                             		sleep(2);
									$stat=checkstatus($id2);
                                     ...
                                     //$ress=file_get_contents("http://aspserver2.info/auto_script/ASP_bot_tl_notify_http.php?sno=$iid&ip=$ip_add&empid=$mailer_id&reason=script_stopped");
										$notification_message = mysql_fetch_array(mysql_query("select `auto_script_status`.`reason` from `svml`.`auto_script_status` where sno='$id2'"));
								        echo $msg = "script is stopped";
										$message = base64_encode("Script Stopped ". strtoupper($notification_message['auto_script_status']));
										`php /var/www/html/notification/send_notification.php $iid $message`;
										// $ch = curl_init();
										// curl_setopt($ch, CURLOPT_URL, "http://$approval_server_ip/auto_script/ASP_bot_tl_notify_http.php?sno=$iid&ip=$ip_add&empid=$m_id&reason=Script_Stopped");
										// curl_setopt($ch, CURLOPT_HEADER, 0);
...
{
	global $retest, $retest_after, $iid;
	if($retest == '1') { $retry_check = -2; }
	end:
	...
	if($query['status']=="0") 
	{
		// Logic  to Retest
		if($retest == '1') {
			if($retry_check > $retest_after) {
				$message = base64_encode("Retesting");
				`php /var/www/html/notification/send_notification.php $iid $message`;
				return 3; 
			} 
		}
