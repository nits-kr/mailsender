...
const buildReplaceTags = (email, opts) => {
  ...
  return (text) => {
    ...
    processed = TagEngine.process(processed, {
      email,
      domain,
      offer_id,
      msgId,
...
const queueEmail = async (email, currentIp, campaignId, opts) => {
  ...
  // 1. Determine/Generate Message-ID
  let finalMsgId = message_id || "";
  if (!finalMsgId) {
    // Generate default pattern 1 if empty
    finalMsgId = generateMessageId(1, domain || "localhost");
  } else if (!isNaN(finalMsgId) && finalMsgId.length < 3) {
    // If it's a number (legacy pattern index), use that
    finalMsgId = generateMessageId(finalMsgId, domain || "localhost");
  }

  // 2. Process MsgID itself through TagEngine (allows randomization like [[mix(10)]]@domain)
  finalMsgId = TagEngine.process(finalMsgId, { email, domain, offer_id });
  ...
  await emailQueue.add("send-email", {
    ...
    charset_alt,
    encoding_alt,
    msgId: finalMsgId, // Pass the pre-generated ID to the worker
    reply_to,
    xmailer,
    offer_id,
