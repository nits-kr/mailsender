...
const DataMerge = () => {
    ...
    const handleMerge = async () => {
        ...
        try {
            const res = await axios.post('/api/data/merge', { filenames });
            setStatus({ type: 'success', message: 'Files merged successfully!' });
            setMergeStats(res.data);
        } catch (error: any) {
            setStatus({ type: 'error', message: error.response?.data?.message || 'Merge failed' });
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="min-h-screen bg-white text-black p-4" style={{ fontFamily: '"Lucida Console", Monaco, monospace' }}>
            <div className="max-w-[1000px] mx-auto">
                <h2 className="text-center text-xl font-bold mb-8 pt-4 uppercase tracking-tighter">DATA MERGE PORTAL</h2>
                ...
                <div className="flex flex-col items-center gap-2 text-[12px] font-bold">
                    ...
                    <p className="text-[10px] text-gray-500 italic mt-1">Note: Files should be present in /var/www/data directory</p>
                    ...
                    <textarea 
                        readOnly
                        value={mergeStats?.output_file || (status.type === 'info' ? 'Processing...' : status.message || 'MERGED OUTPUT FILE NAME WILL APPEAR HERE.....')}
