    ...
    return (
        <div className="min-h-screen bg-white text-black p-4" style={{ fontFamily: '"Lucida Console", Monaco, monospace' }}>
            <div className="max-w-[1000px] mx-auto">
                <h2 className="text-center text-xl font-bold mb-8 pt-4 uppercase tracking-tighter underline">ANALYSIS PORTAL</h2>
                ...
                <div className="border border-gray-400 rounded-sm overflow-hidden bg-white shadow-sm mb-6">
                    <table className="w-full border-collapse text-[12px] font-bold">
                        <thead className="bg-gray-50 border-b border-gray-400">
                            <tr>
                                <th className="px-4 py-2 text-left border-r border-gray-400 w-1/3 italic">Options</th>
                                <th className="px-4 py-2 text-left italic">Selection</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-300">
                            <tr>
                                <td className="px-4 py-3 border-r border-gray-300">Activity Type</td>
                                <td className="px-4 py-3">
                                    <div className="flex gap-6">
                                        <label className="flex items-center gap-2 cursor-pointer">
                                            <input type="checkbox" checked={filters.type.includes('Open')} onChange={() => {}} /> OPENERS
                                        </label>
                                        <label className="flex items-center gap-2 cursor-pointer">
                                            <input type="checkbox" checked={filters.type.includes('Subscribe')} onChange={() => {}} /> CLICKERS
                                        </label>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                <td className="px-4 py-3 border-r border-gray-300">Target Offer</td>
                                <td className="px-4 py-3">
                                    <select 
                                    ...
                                    >
                                        ...
                                        {offers.map(o => (
                                            <option key={o._id} value={o.offer_name}>{o.offer_name}</option>
                                        ))}
                                    </select>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td className="px-4 py-3 border-r border-gray-300">TimeFrame</td>
                                <td className="px-4 py-3">
                                    <select 
                                        className="border border-gray-400 px-1 py-0.5 w-64 outline-none font-mono bg-white"
                ...
                {result && (
                    <div className="border border-gray-400 p-4 font-mono text-sm shadow-sm">
                        <p className="text-xl font-bold uppercase tracking-widest text-center">
                            TOTAL FOUND = <span className="text-blue-600">{result.count.toLocaleString()}</span>
                        </p>
                        <div className="mt-4 text-[10px] text-gray-500 text-center uppercase tracking-widest">
                            * Selected Criteria : {filters.isp || 'ALL ISP'} | {filters.timeframe || 'ALL TIME'}
                        </div>
                    </div>
                )}
            </div>
