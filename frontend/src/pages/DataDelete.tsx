...
const DataDelete = () => {
    ...
    const handleDelete = async (targetFilename?: string) => {
        ...
        if (!fileToDelete) {
            setStatus({ type: 'error', message: 'Enter or select a filename to delete!' });
            return;
        }

        if (!window.confirm(`Are you absolutely sure you want to PERMANENTLY delete ${fileToDelete}?`)) {
            return;
        }
    ...
    return (
        <div className="min-h-screen bg-white text-black p-4" style={{ fontFamily: '"Lucida Console", Monaco, monospace' }}>
            <div className="max-w-[1200px] mx-auto">
                <h2 className="text-center text-xl font-bold mb-8 pt-4 uppercase tracking-tighter">DATA DELETE PORTAL</h2>
                    ...
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <div className="w-full mt-4">
                            <div className="w-full bg-[#5F9EA0] border border-gray-400 p-2 h-[250px] overflow-y-auto font-mono text-[11px] leading-tight">
                                <p className="text-green-900 font-bold mb-1">Delete Process Status...</p>
                                {status.message && (
                                    <p className={status.type === 'error' ? 'text-red-900' : 'text-green-900'}>
                                        [{status.type.toUpperCase()}] {status.message === 'Done' ? 'File erased from system.' : status.message}
                                    </p>
                                )}
                                {loading && (
                                    <div className="text-white mt-2">
                                        <p>Searching for {filename}...</p>
                                        <p className="animate-pulse text-red-100">Wiping data records...</p>
                                    </div>
                                )}
                            </div>
                        </div>
                            ...
                            <table className="w-full text-left">
                                ...
                                <tbody>
                                    {filteredFiles.map((file) => (
                                        <tr key={file.filename} className="hover:bg-red-50 border-b border-gray-200">
                                            <td className="py-2 pr-2 truncate max-w-[200px]">{file.filename}</td>
