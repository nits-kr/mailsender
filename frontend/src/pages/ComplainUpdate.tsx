...
const ComplainUpdate = () => {
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            await axios.post('/api/data/status-update', { ids: idArray, type: 'complain' });
    ...
    return (
        <div className="min-h-screen bg-white text-black p-4" style={{ fontFamily: '"Lucida Console", Monaco, monospace' }}>
            <div className="max-w-[1000px] mx-auto">
                <h2 className="text-center text-xl font-bold mb-8 pt-4 uppercase tracking-tighter">COMPLAIN UPDATE PORTAL</h2>
                ...
                <div className="mt-8 px-4">
                    <div className="w-full bg-[#5F9EA0] border border-gray-400 p-2 h-[300px] overflow-y-auto font-mono text-[11px] leading-tight">
                        <p className="text-green-900 font-bold mb-1">Complain Process Status...</p>
                        {status.message && (
                            <p className={status.type === 'error' ? 'text-red-900' : 'text-green-900'}>
                                [{status.type.toUpperCase()}] {status.message === 'Done' ? 'Records updated in complain database.' : status.message}
                            </p>
                        )}
                        {loading && (
