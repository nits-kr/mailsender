...
const DataSplit = () => {
    ...
    const handleSplit = async () => {
        if (!filename || !count) {
            setStatus({ type: 'error', message: 'Provide filename and split count!' });
            return;
        }

        setLoading(true);
        setStatus({ type: 'info', message: 'Processing split...' });
        setLog(`[INFO] Starting split operation for ${filename}...\n[INFO] Target records per file: ${count}`);
        ...
        try {
            ...
            setStatus({ type: 'error', message: error.response?.data?.message || 'Split failed' });
    ...
    return (
        <div className="min-h-screen bg-white text-black p-4" style={{ fontFamily: '"Lucida Console", Monaco, monospace' }}>
            <div className="max-w-[1000px] mx-auto">
                <h2 className="text-center text-xl font-bold mb-8 pt-4 uppercase tracking-tighter">DATA SPLIT PORTAL</h2>
                ...
                <div className="mt-8 px-4">
                    <div className="w-full bg-[#5F9EA0] border border-gray-400 p-2 h-[400px] overflow-y-auto font-mono text-[11px] leading-tight">
                        <p className="text-green-900 font-bold mb-1">Split Process Status Log...</p>
                        ...
                        {status.message && (
                            <p className={status.type === 'error' ? 'text-red-900' : 'text-green-900'}>
                                [{status.type.toUpperCase()}] {status.message === 'Done' ? 'Split operation finished.' : status.message}
                            </p>
                        )}
                        {loading && (
